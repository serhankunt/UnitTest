...
public class UserServiceTests
{
    ...
    private readonly UpdateUserDto updateUserDto = new(1, "Özge Toksöz", 27, new DateOnly(1996, 06, 30));
    private  User user = new()
    {
        Id = 1,
        Name = "Serhan Kunt",
        Age = 27,
        DateOfBirth = new DateOnly(1996, 09, 11)
    };
    ...
    [Fact]
    public async Task UpdateAsync_ShouldThrowwAndError_WhenUserNotExist()
    {
        //Arrange
    ...
    [Fact]
    public async Task UpdateAsync_ShouldThrowAnError_WhenUserUpdateDetailAreNotValid()
    {
        //Arrange

        //userRepository.GetByIdAsync(updateUserDto.Id).Returns(user);
        //User'lý kullanýmda bu kodu kullanabilirim
        
        userRepository.GetByIdAsync(updateUserDto.Id).Returns(Substitute.For<User>());
    ...
    [Fact]
    public async Task UpdateAsync_ShouldThrowError_WhenUserNameExist()
    {
        //Arrange
        userRepository.NameIsExists(Arg.Any<string>()).Returns(true);
        userRepository.GetByIdAsync(updateUserDto.Id).Returns(user);
    ...
    public async Task UpdateAsync_ShouldLogMessages_WhenInvoked()
    {
        ...
        logger.Received(1).LogInformation(
            "{0} Id'li kullanýcýnýn güncelleme iþlemi {1}ms de baþarýyla tamamlandý",
            Arg.Any<int>(),
            Arg.Any<long>());
    }
    ...
    {
        //Arrange
        var exception = new ArgumentException("Kullanýcý güncelleme esnasýnda bir hatayla karþýlaþtýk");
        userRepository.GetByIdAsync(updateUserDto.Id).Returns(user);
        userRepository.NameIsExists(updateUserDto.Name).Returns(false);
        ...
        logger.Received(1).LogError(Arg.Is(exception), Arg.Is("Kullanýcý güncelleme esnasýnda bir hatayla karþýlaþtýk"));
    }
}
