...
public sealed class UserService(IUserRepository userRepository, ILoggerAdapter<UserService> logger) : IUserService
{
	...
	public async Task<bool> CreateAsync(CreateUserDto request, CancellationToken cancellationToken = default)
	{
		...
		if (nameIsExist)
		{
			throw new ArgumentException("Bu isim daha önce kaydedildi");
		}
		...
		{
			stopWatch.Stop(); 
			logger.LogInformation("User Id : {0} olan kullanıcı {1} ms oluşturuldu",user.Id, stopWatch.ElapsedMilliseconds);
		}
	...
	public async Task<bool> UpdateAsync(UpdateUserDto request, CancellationToken cancellationToken = default)
	{
		...
		logger.LogInformation("{0} kullanıcının güncelleme işlemi yapılmaya başlandı",user.Name);
		var stopWatch = Stopwatch.StartNew();
		try
		{
		...
		{
			logger.LogError(ex, "Kullanıcı güncelleme esnasında bir hatayla karşılaştık");
			throw;
		}
		finally
		{
